...
# Windows-compatible variant (Git Bash)
#
# Two stages:
...
# Cross-platform Python discovery (shared utility)
...
echo "Fetching Gmail..."
if $PYTHON scripts/scan-gmail.py --lookback 1d --json-only > "$GMAIL_OUT" 2>/tmp/gmail-scan-err.log; then
    SURFACED=$($PYTHON -c "import json; print(json.load(open('$GMAIL_OUT'))['total_surfaced'])" 2>/dev/null || echo "?")
