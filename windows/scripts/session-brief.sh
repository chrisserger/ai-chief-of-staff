...
# Entity audit (if script exists) — cross-platform Python discovery
if [ -f "scripts/audit-entities.py" ]; then
    source "$(dirname "$0")/../windows/lib/find-python.sh"

    if [ -n "$PYTHON" ]; then
        ENTITY_SUMMARY=$($PYTHON scripts/audit-entities.py --summary 2>/dev/null)
